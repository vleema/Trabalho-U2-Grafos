use graphs_algorithms::PrimEvent;
...
fn main() {
    println!("Demo Prim - grafo de exemplo (não direcionado, assume conexo)");

    let map = build_test_map();
    let g: AdjacencyList<usize, i32> = AdjacencyList(map);

    let mut it = g.minimum_spanning_tree_prim();
